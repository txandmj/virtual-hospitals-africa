...
async function checkOnboardingStatus(
  trx: TrxOrDb,
  pharmacist_state: PharmacistChatbotUserState,
  if_onboarded_state: PharmacistConversationState,
): Promise<PharmacistConversationState> {
  if (pharmacist_state.chatbot_user.entity_id) {
    return if_onboarded_state
  }
  await conversations.updateChatbotUser(
    trx,
    pharmacist_state.chatbot_user,
    {
      data: {
        ...pharmacist_state.chatbot_user.data,
...
> = {
  'initial_message': {
    type: 'select',
    async prompt(trx: TrxOrDb, pharmacist_state: PharmacistChatbotUserState) {
      const { entity_id } = pharmacist_state.chatbot_user
      if (!entity_id) return welcome

      const pharmacist = await employees.getById(trx, entity_id)
      if (!pharmacist) {
        await conversations.updateChatbotUser(
          trx,
          pharmacist_state.chatbot_user,
  ...
  'not_onboarded:confirm_name': {
    type: 'select',
    async prompt(trx: TrxOrDb, pharmacist_state: PharmacistChatbotUserState) {
      const { licence_number } = pharmacist_state.chatbot_user.data
      assert(typeof licence_number === 'string')
      const pharmacist = await pharmacists.getByLicence(
        trx,
        licence_number,
      )
      assert(
    ...
    options: [
      {
        id: 'Yes',
        title: 'Yes',
        async onExit(trx, pharmacist_state) {
          const { licence_number } = pharmacist_state.chatbot_user.data
          assert(typeof licence_number === 'string')
          const pharmacist = await pharmacists.getByLicence(
            trx,
          ...
          const { after_onboarding_state } = pharmacist_state.chatbot_user.data
  ...
  'not_onboarded:confirm_pharmacy': {
    type: 'select',
    async prompt(trx: TrxOrDb, pharmacist_state: PharmacistChatbotUserState) {
      const { pharmacist_licence_number } = pharmacist_state.chatbot_user.data
      assert(typeof pharmacist_licence_number === 'string')
      const pharmacist = await pharmacists.getByLicence(
        trx,
        pharmacist_licence_number,
      )
      const organization = pharmacist.organizations[0]
      assert(
        organization,
        'The chatbot should not have let the pharmacist proceed with a licence number not corresponding with an extant pharmacy',
      )
      return `We have a record you working at ${organization.name}. Is this correct?`
    },
    options: [
      {
        id: 'Yes',
        ...
        async onExit(trx, pharmacist_state) {
          await conversations.updateChatbotUser(
            ...
            pharmacist_state.chatbot_user,
            {
              data: {
                ...pharmacist_state.chatbot_user.data,
                error_message: 'In that case, something has gone wrong. Please contact your administrator.',
              },
            },
          )
  ...
  'error': {
    type: 'select',
    prompt(_trx: TrxOrDb, pharmacist_state: PharmacistChatbotUserState) {
      const { error_message } = pharmacist_state.chatbot_user.data
      return (
        isString(error_message) ? error_message : 'An error occurred. Please try again.'
      )
    },
    options: [
      {
