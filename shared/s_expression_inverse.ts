...
export function inverseSExpression(node: AnyNode): string {
  switch (node.atom) {
    ...
    case 'finding': {
      ...
      if (node.snomed_concept) parts.push(snomedConceptToString(node.snomed_concept))
      if (node.finding_snomed_concept) parts.push(snomedConceptToString(node.finding_snomed_concept))
      if (node.value_snomed_concept) parts.push(snomedConceptToString(node.value_snomed_concept))
    ...
    case 'attribute': {
      return `(attribute ${snomedConceptToString(node.relation_snomed_concept)} ${snomedConceptToString(node.finding_snomed_concept)})`
    }
    ...
    case 'task': {
      return `(task ${inverseSExpression(node.left)} ${inverseSExpression(node.right)})`
    }
