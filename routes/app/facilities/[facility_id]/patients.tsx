...
export const handler: LoggedInHealthWorkerHandlerWithProps<PatientsProps> = {
  async GET(req, ctx) {
    ...
    const patients_with_href = patients.map((patient) => {
      const patient_in_waiting_room = room.find((entry) => entry.patient.id === patient.id)
      if (patient_in_waiting_room) {
        return {id: patient.id , name: patient.name, href: `/app/patients/${patient.id}`}
      } else{
        return {id: patient.id , name: patient.name, href: `/app/facilities/${facility_id}/waiting_room/add?patient_id=${patient?.id}`}
      }
    })
