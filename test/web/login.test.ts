...
describeWithWebServer('/login', 8002, (route) => {
  ...
  describe('when logged in', () => {
    ...
    itUsesTrxAnd(
      'starts in an empty waiting room with sidebar links',
      async (trx) => {
        const mock = await addTestHealthWorkerWithSession(trx)
        const admin = await upsertWithGoogleCredentials(trx, testHealthWorker())
        await employee.add(trx, [{
    ...
    itUsesTrxAnd(
      'allows a health worker employed at a facility to view/approve its employees',
      async (trx) => {
        const mock = await addTestHealthWorkerWithSession(trx)
    ...
    itUsesTrxAnd(
      "doesn't allow unemployed access to employees",
      async (trx) => {
        const mock = await addTestHealthWorkerWithSession(trx)
        await employee.add(trx, [{
          facility_id: 1,
