...
describe(
  '/app/facilities/[organization_id]/patients',
    ...
    it("can search for patients, sending back an appropriate href depending on whether the patient is in the organization's waiting room", () =>
      withTestFacility(db, async (organization_id) => {
        ...
        const response = await fetch(
          `${route}/app/facilities/${organization_id}/patients?search=${unique_name}`,
          {
            headers: {
        ...
        assertEquals(json, [
          {
            ...
            href:
              `/app/facilities/${organization_id}/waiting_room/add?patient_id=${patient_not_in_waiting_room.id}`,
          },
          {
