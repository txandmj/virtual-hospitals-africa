...
import { PartialPatientDemographics, insertPatientSeekingTreatmentWithEmployeeAndCompleteRegistrationForTest } from '../../../../_helpers/workflows.ts'
import { route } from '../../../../route.ts'
import fromEntries from '../../../../../util/fromEntries.ts'
import { WARNING_SIGNS } from '../../../../../shared/warning_signs.ts'
import { VitalMeasurement, VitalAssessment } from '../../../../../shared/vitals.ts'
 ...
 */
export async function setupTriage({ patient_demographics, warning_signs, conditions, vitals }: TriageScenario) {
  ...
  const warning_signs_post_data = fromEntries(warning_signs.map((warning_sign) => 
    [warning_sign, WARNING_SIGNS[warning_sign].clinical_finding_s_expression]
  ))
  ...
  if (conditions) {
    const conditions_post_data = fromEntries(conditions.map(condition => ([condition, { existence: 'Yes' }])))
