...
// import { createEventProcessor, type EventProcessor } from '../../events/processor.ts'
...
//     })
//     afterAll(() => db.destroy())
//     afterAll(() => processor.exit({ graceful: false }))
//     afterAll(() => events.closeAllProcessedPubSub({ graceful: false }))

//     describeParallel('addListeners', () => {
//       itParallel(
//         'adds a row for each event listener that we want to run',
//         async () => {
...
//       itParallel(
//         'marks the listener as having errored, when that is the case',
//         async () => {
//           const bar = generateUUID()
//           const event = await events.insert(db, {
//             type: 'TEST_NEVER_WORKS',
...
//       itParallel(
//         'does not retry immediately, respecting the backoff_until. It will not retry after 3 errors',
//         async () => {
//           const bar = generateUUID()
//           const event = await events.insert(db, {
//             type: 'TEST_NEVER_WORKS',
...
//           await events.clearBackoff(db, {
//             event_listener_id: listener_after.id,
//           })
...
//       itParallel(
//         'can move into a successful state for a message that fails on the first try',
//         async () => {
//           const event = await events.insert(db, {
