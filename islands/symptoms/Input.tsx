import {
  ...
  export default function SymptomInput({
    ...
    const media = useSignal(value
      ? {
        mime_type: value?.media?.[0]?.mime_type,
        url: value?.media?.[0]?.url,
        name: `${name}.media.0`,
        file: value?.media?.[0]?.mime_type ? new File([], '', { type: value?.media?.[0]?.mime_type }) : undefined,
      }
      : null
    )
    ...
    return (
      <RemoveRow onClick={remove} labelled>
        <div className='flex flex-col space-y-1 w-full'>
          <input name={`${name}.patient_symptom_id`} type='hidden' value={value?.patient_symptom_id} />
