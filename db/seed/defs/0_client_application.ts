...
export default create(
  ['ClientApplication'],
  async (db: Kysely<any>) => {
    const project = await db.selectFrom('Project').selectAll().executeTakeFirstOrThrow()
    const practitioner = await db.selectFrom('Practitioner')
      .selectAll()
      .where(sql<boolean>`(cast(content AS json)->'name')::text = '[{"given":["Medplum"],"family":"Admin"}]'`)
