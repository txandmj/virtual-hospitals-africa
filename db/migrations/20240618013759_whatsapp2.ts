...
export async function up(db: Kysely<unknown>) {
  ...
  await createStandardTable(db, 'patient_whatsapp_messages_received', qb => 
    qb.addColumn('whatsapp_message_received_id', 'uuid', col => col.references('whatsapp_messages_received.id').onDelete('cascade'))
  ...
  await createStandardTable(db, 'pharmacist_whatsapp_messages_received', qb => 
    qb.addColumn('whatsapp_message_received_id', 'uuid', col => col.references('whatsapp_messages_received.id').onDelete('cascade'))
      .addColumn('pharmacist_id', 'uuid', col => col.references('patients.id').onDelete('cascade'))
