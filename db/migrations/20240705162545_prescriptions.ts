...
export async function up(db: Kysely<unknown>) {
  await createStandardTable(db, 'prescriptions', (qb) =>
    qb
      ...
      .addColumn('prescriber_id', 'uuid', (col) => col.notNull().references('patient_encounter_providers.id').onDelete('cascade'),)
      .addColumn('patient_id', 'uuid', (col) => col.references('patients.id').onDelete('cascade'),)
    )

    await createStandardTable(db, 'patient_prescription_medications', (qb) =>
      qb.addColumn(
        'patient_condition_medication_id',
        'uuid',
        (col) => col.notNull().references('patient_condition_medications.id').onDelete('cascade'),
      )
        .addColumn(
          'prescription_id',
          'uuid',
  ...
  await createStandardTable(db, 'patient_prescription_medications_filled', (qb) =>
      qb.addColumn(
        'patient_prescription_medication_id',
        'uuid',
        (col) => col.notNull().references('patient_prescription_medications.id').onDelete('cascade'),
      )
        ...
        .addUniqueConstraint('patient_prescription_medication_id', ['patient_prescription_medication_id'])
    )
}
