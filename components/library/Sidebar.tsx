  ...
  return (
    <li>
      <a
        href={href}
        className={cls(
          'hover:text-gray-900 hover:bg-gray-50 group flex gap-x-3 items-center rounded-md p-2 text-sm leading-6',
...
const matchingSidebarLink = matchActiveLink(navLinks)
      ...
      <div className='flex flex-auto flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-5 pb-4'>
        ...
        <nav className='flex flex-1 flex-col'>
          <ul role='list' className='-mx-2 space-y-1'>
            {navLinks.map((link) => (
              <NavItem {...link} active={link === activeLink} />
            ))}
          </ul>
        </nav>
      </div>
