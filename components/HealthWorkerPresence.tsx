...
export function HealthWorkerPresence({ employee }: {
  ...
  const appt_time = appt ? new Date(appt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : null
  ...
  return (
    <span className='flex flex-col items-start gap-1 mt-2 sm:mt-0 sm:items-end'>
      ...
      {appt_time && <span className='text-xs text-amber-600'>Appt at {appt_time}</span>}
    </span>
  )
}
