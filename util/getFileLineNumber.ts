...
function extractLineInfo(caller_line: string) {
  ...
  if (match_with_function_name) {
    const [_match, function_name, file_name, line_number, column_number] = match_with_function_name
  ...
  const [_match, file_name, line_number, column_number] = match_without_function_name
...
export function getCaller(up_stack_levels = 0) {
  try {
    ...
    const { file_name, function_name, line_number, column_number } = extractLineInfo(
      caller_line,
    )

    const without_cwd = file_name.replace('file://', '').replace(cwd, '')
    const pretty_file_name = without_cwd.startsWith('/') ? without_cwd.slice(1) : without_cwd
